...
그래서 `res.ok`(상태 코드 200~299)를 직접 확인해야 합니다. 실무에서는 이 처리를 한 곳에 모아 두고 씁니다 — 37장의 API 레이어가 그 이야기입니다.
...
직접 쓸 일은 드뭅니다. TanStack Query 같은 라이브러리가 관리해 주거든요(34장). 다만 **라이브러리가 건네주는 `signal`을 실제 요청에 넘겨야** 취소가 전달됩니다. 그냥 쓰면 화면만 사라지고 요청은 계속 날아갑니다. 다만 **"화면이 사라졌는데 응답이 늦게 도착하는 상황"이 실재한다** 는 건 알아 두세요. 2장의 "오래 사는 프로세스"가 만드는 문제 중 하나입니다.
