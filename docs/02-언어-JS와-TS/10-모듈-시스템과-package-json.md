...
Java의 `import`와 결정적으로 다른 점: **가리키는 대상이 두 종류** 라는 것입니다. `./`나 `../`로 시작하면 파일 경로이고, 그렇지 않으면 패키지 이름이거나 프로젝트가 설정한 별칭입니다.
...
`./`나 `../`로 시작하지 않으면 번들러는 먼저 **프로젝트가 설정한 별칭인지** 보고, 아니면 **패키지**로 간주해 `node_modules`에서 찾습니다. `@/`로 시작하는 것이 대표적인 별칭인데, `../../../utils/date` 같은 지옥을 피하려고 씁니다. Next.js는 기본으로 제공합니다.
