...
함수 컴포넌트는 그냥 함수라서 원래는 아무것도 기억하지 못합니다. 값을 기억하려면 어딘가에 맡겨야 하고, 외부 시스템과 연결하려면 누군가에게 부탁해야 합니다.

**Hooks는 그 부탁을 하는 창구입니다.** 렌더 도중에 React에게 "이 값을 나 대신 들고 있어줘", "이 정리 작업을 나중에 실행해줘"라고 등록하는 함수 호출입니다.

> Spring의 `@Transactional` 같은 애너테이션과는 다릅니다. 그쪽은 프레임워크가 당신 코드를 감싸는 것이고, Hooks는 **당신이 직접 부르는 함수** 입니다. 그래서 호출 위치에 규칙이 붙습니다.
...
> **예외가 하나 있습니다.** React 19에 추가된 `use`는 이름이 `use`로 시작하지만 훅이 아니고, 조건문 안에서 호출해도 됩니다. 이 문서에서는 쓰지 않으니, 남의 코드에서 보면 "이건 규칙이 다른 놈"이라고만 알아두세요.
...
의존성 배열에 따라 동작이 달라집니다. 다만 `[]`가 **앱 전체에서 딱 한 번** 을 뜻하지는 않습니다. 그 컴포넌트가 화면에서 사라졌다 다시 나타나면 또 실행됩니다. (개발 모드에서는 실수를 잡으려고 일부러 한 번 더 돌리기도 합니다 — 심화 가이드 8장.)

```tsx
useEffect(() => { ... });            // 매 렌더마다 (거의 안 씀)
useEffect(() => { ... }, []);        // 화면에 붙을 때 (아래 주의)
...
> **규칙**: `useEffect` 안에서 **뒤에 계속 살아남는 것을 만들었다면** 그것을 없애는 코드를 반환하세요. 이벤트 리스너, 타이머, 웹소켓 연결, 구독이 여기 해당합니다.
>
> 반대로 `document.title`을 바꾸는 것처럼 **한 번 하고 끝나는 일** 에는 정리 함수가 필요 없습니다. 기준은 "이게 뒤에 뭔가를 남기는가"입니다.
...
`useEffect` 버전은 렌더가 두 번 일어나고(orders 갱신 → effect → total 갱신), 중간에 값이 어긋나는 순간이 생깁니다. **다른 값으로부터 계산되는 것은 state가 아닙니다.** 33장에서 다시 다룹니다.
...
직접 다 처리하는 대신 도구를 씁니다. **Next.js 서버 컴포넌트**(31장)나 **TanStack Query**(34장)가 이 문제를 통째로 해결합니다.
...
객체와 함수를 의존성에 넣으면 8장에서 배운 참조 비교 때문에 매번 다릅니다. 해법은 **원시값만 의존성에 넣거나**, `useMemo`/`useCallback`으로 참조를 고정하는 것입니다. 후자는 25장에서 다룹니다.
...
function useWindowWidth() {
  const [width, setWidth] = useState(0);   // 초기값은 0. 이유는 아래에
  useEffect(() => {
    const onResize = () => setWidth(window.innerWidth);
    onResize();                             // 브라우저에서 처음 한 번 측정
    window.addEventListener('resize', onResize);
    return () => window.removeEventListener('resize', onResize);
  }, []);
  return width;
...
> **`useState(window.innerWidth)`라고 쓰지 않은 이유.** 이 코드는 서버에서도 한 번 실행될 수 있습니다(Part 5에서 다룹니다). 서버에는 `window`가 없어서 그 자리에서 터집니다. 그래서 **`window`를 만지는 코드는 `useEffect` 안에 두는 것이 원칙** 입니다. 대신 첫 화면에는 잠깐 0이 보였다가 측정값으로 바뀝니다.
...
`useEffect`를 생명주기 훅으로 취급하면 남용하게 됩니다. "언제 실행할지"를 지정하는 도구가 아니라, **"무엇과 맞춰 둘지"** 를 지정하는 동기화 도구입니다.
...
없습니다. 로직만 공유됩니다. 상태 공유는 상태 끌어올리기나 Context입니다(35장).
