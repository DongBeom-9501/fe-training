...
> **`useState(window.innerWidth)`라고 쓰지 않은 이유.** 이 코드는 서버에서도 한 번 실행될 수 있습니다(31장에서 다룹니다). 서버에는 `window`가 없어서 그 자리에서 터집니다. 그래서 **`window`를 만지는 코드는 `useEffect` 안에 두는 것이 원칙** 입니다. 대신 첫 화면에는 잠깐 0이 보였다가 측정값으로 바뀝니다.
