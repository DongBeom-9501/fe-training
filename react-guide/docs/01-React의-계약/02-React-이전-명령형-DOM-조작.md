...
`tbody.innerHTML = ''` 로 통째로 지우고 다시 그리는 이유가 있습니다. **"무엇이 바뀌었는지"를 계산하는 게 너무 어렵기 때문입니다.** 그래서 다 지우고 다시 그리는데, 그러면 사용자가 입력하던 값이나 스크롤 위치, 포커스가 날아갑니다.
...
반대에 가깝습니다. React는 **DOM 조작을 못 하게 막고** 상태만 다루게 만드는 라이브러리입니다. 그 제약이 이득의 원천입니다.
